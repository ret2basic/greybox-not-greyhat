...
candidate, checks response `amountIn`, route mints/chains, recipient, and quote
container count against the approved request when the response exposes those
fields, prints the exact policy and decode follow-up commands, and still writes
no official sidecars. Stdin is supported for only one side at a time; the other
sides must be files so the tool never tries to read the same stream twice.
