...
BeraChain Web/Apps 的 bounty scope 已完整归档，16 个显式资产均完成基础盘点；目前有 1 项经内部技术验证的 Medium Web finding，但尚未记录 Immunefi submission ID、平台确认或披露批准。完整资产名、根因、payload、PoC 和现场证据只保存在私密交接包中，不在本公开文档披露。
...
当前只有“内部已验证”的记录；没有证据表明已经提交或被平台接受。
...
- 已验证 1 项 Medium Web finding；为遵守未修复漏洞披露规则，本公开仓库只记录数量、级别和状态。
...
- 人工维护的 coverage/research ledger 更新到 2026-07-10。
- 旧 `scope-policy.json` 和 `lead-portfolio.json` 仍可能保留 2026-07-08 的 12-asset/partial 状态，恢复后应离线重建。
...
1. 在私密材料中对已验证 Medium 做最后一次 scope、impact、PoC、负面对照和披露规则复核，然后通过 Immunefi 私密渠道提交，并保存 submission/ticket 状态。
2. 离线重建完整 16-asset 的 scope policy 和 lead portfolio，消除旧 partial 状态。
